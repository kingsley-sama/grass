...
async def connect_to_wss(socks5_proxy, user_id):
    random_word = "bomrexx"
    device_id = str(uuid.uuid3(uuid.NAMESPACE_DNS, random_word + socks5_proxy))
...
async def main():
    # Input User_id
    _user_id = "2eMSnl1Z5tpzeB9Ke74u6JYSBs6"
    # Load Your Socks Proxies file
    with open("socks5_proxies.txt", "r") as f:
        socks5_proxy_list = [line.strip() for line in f]
    tasks = [
        asyncio.ensure_future(connect_to_wss(i, _user_id)) for i in socks5_proxy_list
    ]
